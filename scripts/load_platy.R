...
gl_vcf <- gl.read.vcf(vcffile =  paste0(chrom_dir, chrom_gl, ".vcf"),
                      verbose = 0)
# filtering
...
samples_info_ox <-
  read.csv(paste0(path_dir, "/info/", "locations_oxford.csv"))
# decompressing vcf
system(paste0("gzip -dk ", chrom_dir,"Martin_" ,chrom, ".vcf.gz"))
file_info <- file.info(paste0(chrom_dir,"Martin_", chrom, ".vcf"))
if(file_info$size > 14000){
  
  gl_vcf_ox_new <- gl.read.vcf(paste0(chrom_dir,"Martin_", chrom, ".vcf"),verbose = 0)
  # filtering
  # names(gl_vcf_ox_new$other$loc.metrics)
  gl_vcf_ox_new$other$loc.metrics$QUAL <-
    as.numeric(gl_vcf_ox_new$other$loc.metrics$QUAL)
  #gl.report.locmetric(gl_vcf_ox_new,metric ="QUAL")

  # assigning information to individuals and populations
  gl_vcf_ox_new$other$ind.metrics <- samples_info_ox
  pop(gl_vcf_ox_new) <- gl_vcf_ox_new$other$ind.metrics$Region
  
  gl_ox_new <- gl_vcf_ox_new
  
  cat("  Object 'gl_ox_new' created with", nLoc(gl_ox),"loci\n")
  
}else{
  cat(" No loci in the Oxford new dataset for this chromosome\n\n")
  gl_ox_new <- NULL
}
...
if(length(locNames(x_dart)[t2])>0){
