...
load_platy_3 <- function(chrom){
  ...
  chrom_gl <- paste0("Martin_chr_", chrom)
  ...
  # decompressing vcf
  system(paste0("gzip -dk ", chrom_dir, chrom_gl, ".vcf.gz"))
  gl_vcf <- gl.read.vcf(paste0(chrom_dir, chrom_gl, ".vcf"),verbose = 0)
  # filtering
  ...
  ########################################################################
    ...
    # 
    # 
    # x_ox <- gl.filter.callrate(gl_ox, threshold = 1,verbose=0)
    # 
    # x_martin <- gl.filter.callrate(gl_26, threshold = 1,verbose = 0)
    # 
    # x_martin$position <- as.integer(x_martin$position-1)
    # 
    # x_ox$loc.names <- as.character(x_ox$position)
    # x_martin$loc.names <- as.character(x_martin$position)
    # 
    # genotypes_2_datasets(x=x_ox,y=x_martin)
    # 
    # x_ox$position <-  as.integer(x_ox$position +1)
    # 
    # t2 <- which(x_ox$position %in% x_martin$position)
    # t3 <- which(x_martin$position %in% x_ox$position)
    # 
    # if(length(locNames(x_ox)[t2])>0){
    # 
    #   x_ox <- gl.keep.loc(x_ox, loc.list = locNames(x_ox)[t2],verbose = 0)
    #   x_ox <- x_ox[, order(x_ox$position)]
    #   x_ox <- x_ox[order(indNames(x_ox)), ]
    # 
    #   x_martin <- gl.keep.loc(x_martin, loc.list = locNames(x_martin)[t3],verbose = 0)
    #   x_martin <- x_martin[, order(x_martin$position)]
    #   x_martin <- x_martin[order(indNames(x_martin)), ]
    # 
    #   s1 <- as.matrix(x_ox)
    #   s2 <- as.matrix(x_martin)
    #   
    #   colnames(s1) <- as.character(x_ox$position)
    #   colnames(s2) <- as.character(x_martin$position)
    #   
    # 
    #   s1[s1 == 2] <- 0
    #   s2[s2 == 2] <- 0
    # 
    #   # x_ox$position == x_martin$position
    # 
    #   # percentage of incorrect called genotypes between DArT and new dataset
    #   genotypes <- nInd(x_martin) * nLoc(x_martin)
    ...
    # }else{
    #   cat("No loci in common between the datasets\n\n")
    # }
